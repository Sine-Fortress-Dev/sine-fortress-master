...
void C_TFTeam::UpdateTeamName( void )
{
	...
	if ( TFGameRules() )
	{
		if( TFGameRules()->IsInTournamentMode() && ( ( m_iTeamNum == TF_TEAM_RED ) || ( m_iTeamNum == TF_TEAM_BLUE ) ) )
		{
		...
		{
			if ( m_iTeamNum == TF_TEAM_BLUE && TFGameRules()->GetBlueTeamHasCustomName() )
			{
				const char* name = TFGameRules()->GetBlueTeamName();
				g_pVGuiLocalize->ConvertANSIToUnicode( name, m_wzTeamname, sizeof( m_wzTeamname ) );
				return;
			}
			else if ( m_iTeamNum == TF_TEAM_RED && TFGameRules()->GetRedTeamHasCustomName() )
			{
				const char* name = TFGameRules()->GetRedTeamName();
				g_pVGuiLocalize->ConvertANSIToUnicode( name, m_wzTeamname, sizeof( m_wzTeamname ) );
				return;
			}
		}
